# API доступен публично! 🚀

## Публичный URL

**API доступно по адресу:** `http://45.12.136.239:8000`

## Endpoints

### 1. Health Check
```bash
curl http://45.12.136.239:8000/
```

**Ответ:**
```json
{
  "status": "ok",
  "service": "AmWine Recommendations API",
  "version": "1.0.0"
}
```

### 2. Получить рекомендации
```bash
curl -X POST "http://45.12.136.239:8000/offers/recommend" \
  -H "Content-Type: application/json" \
  -d '{
    "occasion": "🎉 Вечеринка",
    "style": "🌤 Легко и мягко",
    "drink_types": ["🍷 Вино / игристое", "🍺 Пиво / сидр"],
    "tastes": ["🍑 Фруктовое / ароматное", "🍬 Сладковатое"],
    "people_count": 6,
    "budget": "💰 1000–3000 ₽"
  }'
```

### 3. Интерактивная документация

- **Swagger UI:** http://45.12.136.239:8000/docs
- **ReDoc:** http://45.12.136.239:8000/redoc

## Использование в приложении

### JavaScript / TypeScript

```typescript
const response = await fetch('http://45.12.136.239:8000/offers/recommend', {
  method: 'POST',
  headers: {
    'Content-Type': 'application/json',
  },
  body: JSON.stringify({
    occasion: "🎉 Вечеринка",
    style: "🌤 Легко и мягко",
    drink_types: ["🍷 Вино / игристое"],
    tastes: ["🍑 Фруктовое / ароматное"],
    people_count: 6,
    budget: "💰 1000–3000 ₽"
  })
});

const data = await response.json();
console.log(data.offers); // Array of 10 products
```

### React Native / Expo

```typescript
import axios from 'axios';

const getRecommendations = async (quizData) => {
  try {
    const response = await axios.post(
      'http://45.12.136.239:8000/offers/recommend',
      quizData
    );
    return response.data.offers;
  } catch (error) {
    console.error('Error fetching recommendations:', error);
    throw error;
  }
};

// Использование:
const offers = await getRecommendations({
  occasion: "🎉 Вечеринка",
  style: "🌤 Легко и мягко",
  drink_types: ["🍷 Вино / игристое"],
  tastes: ["🍑 Фруктовое / ароматное"],
  people_count: 6,
  budget: "💰 1000–3000 ₽"
});
```

## Формат ответа

```json
{
  "offers": [
    {
      "id": "SKU123",
      "description": "Название вина",
      "image": "https://url-to-image.jpg",
      "url": "https://product-page-url",
      "price_raw": "999 ₽"
    }
    // ... еще 9 товаров
  ]
}
```

## Примечания

⚠️ **Важно:**
- API возвращает случайные товары, т.к. база данных пока пустая
- После загрузки товаров в БД (через `db/update_products.py`) API будет возвращать реальные товары
- ML-фильтрация будет добавлена позже в модуле `ml/recsys`

## Управление сервисом

API запущен как systemd service и автоматически стартует при перезагрузке сервера.

**Просмотр статуса:**
```bash
sudo systemctl status amwine-api
```

**Просмотр логов:**
```bash
sudo journalctl -u amwine-api -f
```

**Перезапуск:**
```bash
sudo systemctl restart amwine-api
```

**Остановка:**
```bash
sudo systemctl stop amwine-api
```

## Безопасность в Yandex Cloud

Если API недоступно извне, откройте порт 8000 в группе безопасности:

1. Зайдите в Yandex Cloud Console
2. Выберите вашу VM (alco-vm)
3. Перейдите в "Группы безопасности"
4. Добавьте правило:
   - **Направление:** Входящий трафик
   - **Протокол:** TCP
   - **Порт:** 8000
   - **CIDR:** 0.0.0.0/0 (или конкретные IP для безопасности)
5. Сохраните изменения

## Следующие шаги

1. ✅ API запущен и доступен публично
2. 📝 Загрузите товары в БД через `db/update_products.py`
3. 🧪 Протестируйте из вашего приложения
4. 🤖 Добавьте ML-модель для умной фильтрации (опционально)
